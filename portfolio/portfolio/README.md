...
#### 2. **메인 화면**
    ...
    - 이름과 직업 타이틀
    - 매력적인 자기소개
    - CTA 버튼들 (프로젝트보기, 이력서 다운로드)
    - 소셜 미디어 링크

#### 3. **소개 섹션**
    - 상세한 자기소개 텍스트
    - 완성 프로젝트 수, 팔로워, Github 커밋 수, 연속 학습일 수
...
- **모던 디자인**: CSS Gird, Flex box, 그라디언트 활용
- **애니메이션**: 스크롤 애니메이션, hover 효과
- **일관성**: 통일된 색상 팔레트와 타이포 그래피
...
2. **인터렉티브 기능**
    ```JavaScript
        function clearDisplay(){ //화면을 완전히 지우는 함수
            const display = document.getElementById("display"); //DOM에서 디스플레이 요소 가져오기
            
            currentInput = '';//모든 값 초기화, 입력 변수 비우기
...
3. **애니메이션 구현**
    ```JavaScript
        function backSpace() { //마지막 입력을 지우는 함수
            const display = document.getElementById("display"); //DOM에서 디스플레이 요소 가져오기
            
            /* slice() : 마지막 문자 제외하고 자르기 */
            currentInput = currentInput.slice(0,-1); //현재 입력값에서 마지막 문자 제거
            display.value = currentInput; //화면에 업데이트된 계산식 표시
...
### ✍️ **제출 사항**
1. **소스코드**: 모든 HTML, CSS, JS파일
2. **README.md**: 프로젝트 소개, 기능 설명, 설치 및 실행 방법
3. **개발 일지**: 주차별 진행사항, 내용, 트러블슈팅
4. **스크린샷** 
